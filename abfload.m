...
if ischar(channels)
  if strcmp(channels,'a')
    ...
    fclose(fid);
    ermsg=['input parameter ''channels'' must either be a cell array ',...
        'holding channel names or the single character ''a'' (=all channels)'];
    error(ermsg);
  end
else
...
switch h.nOperationMode
  case 1
    ...
    if h.fFileVersionNumber>=2.0
      ermsg=['abfload currently does not work with data acquired in event-driven',...
          'variable-length mode and ABF version 2.0'...
          ,'ABF version issue'];
      errordlg(ermsg);
    else
      ...
      if doLoadData
        for i=1:nSweeps,
          ...
          if n~=segLengthInPts(sweeps(i)),
            warning(['something went wrong reading episode ' int2str(sweeps(i)) ': '...
                segLengthInPts(sweeps(i)) ' points should have been read, ' int2str(n) ' points actually read']);
          end
          ...
          if ~h.nDataFormat
            for j=1:length(chInd),
              ...
              tmpd(:,j)=tmpd(:,j)/(h.fInstrumentScaleFactor(ch)*h.fSignalGain(ch)*h.fADCProgrammableGain(ch)...
                  *addGain(ch))*h.fADCRange/h.lADCResolution+h.fInstrumentOffset(ch)-h.fSignalOffset(ch);
            end
          end
    ...
    if doLoadData
      for i=1:nSweeps,
        ...
        if n~=dataPtsPerSweep,
          fclose(fid);
          error(['something went wrong reading episode ' int2str(sweeps(i)) ': '...
              dataPtsPerSweep ' points should have been read, ' int2str(n) ' points actually read']);
        end
        h.dataPtsPerChan=n/h.nADCNumChannels;
        if rem(n,h.nADCNumChannels)>0
          fclose(fid);
    ...
    if doLoadData
      ...
      if length(chInd)==1 && h.nADCNumChannels>1
        ...
        if n~=h.dataPtsPerChan,
          fclose(fid);
          error(['something went wrong reading file (' int2str(h.dataPtsPerChan)...
              ' points should have been read, ' int2str(n) ' points actually read']);
        end
      elseif length(chInd)/h.nADCNumChannels<1
        ...
        if n~=h.dataPts,
          fclose(fid);
          error(['something went wrong reading file (' int2str(h.dataPts)...
              ' points should have been read, ' int2str(n) ' points actually read']);
        end
        % separate channels..
      ...
      if ~h.nDataFormat
        for j=1:length(chInd),
          ch=recChIdx(chInd(j))+1;
          d(:,j)=d(:,j)/(h.fInstrumentScaleFactor(ch)*h.fSignalGain(ch)*h.fADCProgrammableGain(ch)...
              *addGain(ch))*h.fADCRange/h.lADCResolution+h.fInstrumentOffset(ch)-h.fSignalOffset(ch);
        end
      end
    end
